...
//
// Hand-checked tiny case (N=1, C=1, H=W=4) plus a few larger shapes. Output
// channels are stacked as c_out = (dh*2 + dw)*C + c_in over the 2x2 block
// (dh, dw) at each output spatial location.
...
void test_hand_checked() {
    ...
    Tensor Xt = Tensor::from_host_on(brotensor::Device::CPU,
                                     X.data(), 1, 16);
    Tensor Y;
    brotensor::spatial_merge_2x2_forward(Xt, 1, 1, 4, 4, Y);
    CHECK(Y.rows == 1);
    CHECK(Y.cols == 4 * 2 * 2);
...
void run_shape(const char* label, int N, int C, int H, int W, uint64_t seed) {
    ...
    ref_spatial_merge(X, N, C, H, W, Y_ref);

    Tensor Xt = Tensor::from_host_on(brotensor::Device::CPU,
                                     X.data(), N, C * H * W);
    Tensor Y;
    brotensor::spatial_merge_2x2_forward(Xt, N, C, H, W, Y);
    CHECK(Y.rows == N);
...
void test_odd_dim_rejected() {
    ...
    bool threw = false;
    try {
        brotensor::spatial_merge_2x2_forward(X, 1, 1, 3, 4, Y);
