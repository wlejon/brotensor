...
void run_mha_with_bias(int K, int D, int H, uint64_t seed,
    ...
    Tensor bq = Tensor::mat(D, 1), bk = Tensor::mat(D, 1),
           bv = Tensor::mat(D, 1), bo = Tensor::mat(D, 1);
    fill_random(bq, rng, 0.3f);
    fill_random(bk, rng, 0.3f);
    fill_random(bv, rng, 0.3f);
    fill_random(bo, rng, 0.3f);
    ...
    // Pre-fill db* to validate accumulation.
    Tensor dbq_init = Tensor::mat(D, 1), dbk_init = Tensor::mat(D, 1),
           dbv_init = Tensor::mat(D, 1), dbo_init = Tensor::mat(D, 1);
    fill_random(dbq_init, rng, 0.1f);
    fill_random(dbk_init, rng, 0.1f);
    fill_random(dbv_init, rng, 0.1f);
    fill_random(dbo_init, rng, 0.1f);
    ...
    Tensor O_cpu = Tensor::mat(K, D);
    brotensor::mha_forward(
        X, Wq, Wk, Wv, Wo, &bq, &bk, &bv, &bo, host_mask, H,
        Qh, Kh, Vh, Attnh, Yconcat, O_cpu);

    Tensor dX_cpu = Tensor::mat(K, D);
    Tensor dWq_cpu = dWq_init, dWk_cpu = dWk_init,
           dWv_cpu = dWv_init, dWo_cpu = dWo_init;
    Tensor dbq_cpu = dbq_init, dbk_cpu = dbk_init,
           dbv_cpu = dbv_init, dbo_cpu = dbo_init;
    brotensor::mha_backward(
        dO, X, Qh, Kh, Vh, Attnh, Yconcat,
        Wq, Wk, Wv, Wo, host_mask, H,
        dX_cpu, dWq_cpu, dWk_cpu, dWv_cpu, dWo_cpu,
        &dbq_cpu, &dbk_cpu, &dbv_cpu, &dbo_cpu);
    ...
    const float* d_mask = static_cast<const float*>(d_mask_buf.data);
    brotensor::mha_forward(
        gX, gWq, gWk, gWv, gWo, &gbq, &gbk, &gbv, &gbo, d_mask, H,
        gQh, gKh, gVh, gAttnh, gYconcat, gO);
    ...
    Tensor gdWv = dWv_init.to(gpu_device());
    Tensor gdWo = dWo_init.to(gpu_device());
    Tensor gdbq = dbq_init.to(gpu_device());
    Tensor gdbk = dbk_init.to(gpu_device());
    Tensor gdbv = dbv_init.to(gpu_device());
    Tensor gdbo = dbo_init.to(gpu_device());
    ...
    const float atol = 5e-5f, rtol = 5e-4f;
    compare_tensors(O_cpu,  O_gpu,  "mhab.O", atol, rtol);
    compare_tensors(dX_cpu, download_to_host(gdX), "mhab.dX", atol, rtol);
    compare_tensors(dWq_cpu, download_to_host(gdWq), "mhab.dWq", atol, rtol);
    compare_tensors(dWk_cpu, download_to_host(gdWk), "mhab.dWk", atol, rtol);
    compare_tensors(dWv_cpu, download_to_host(gdWv), "mhab.dWv", atol, rtol);
    compare_tensors(dWo_cpu, download_to_host(gdWo), "mhab.dWo", atol, rtol);
    compare_tensors(dbq_cpu, download_to_host(gdbq), "mhab.dbq", atol, rtol);
    compare_tensors(dbk_cpu, download_to_host(gdbk), "mhab.dbk", atol, rtol);
    compare_tensors(dbv_cpu, download_to_host(gdbv), "mhab.dbv", atol, rtol);
    compare_tensors(dbo_cpu, download_to_host(gdbo), "mhab.dbo", atol, rtol);
}
...
// Bias variants (Q/K/V/O biases all non-null).
BT_PARITY_TEST(mha_bias_h2_K4_D32)       { run_mha_with_bias(4,  32, 2, 0x620ull, nullptr); }
BT_PARITY_TEST(mha_bias_h4_K16_D64)      { run_mha_with_bias(16, 64, 4, 0x621ull, nullptr); }
BT_PARITY_TEST(mha_bias_h2_K4_D32_mask)  { auto m = partial_mask(4);  run_mha_with_bias(4,  32, 2, 0x622ull, &m); }
BT_PARITY_TEST(mha_bias_h4_K16_D64_mask) { auto m = partial_mask(16); run_mha_with_bias(16, 64, 4, 0x623ull, &m); }
