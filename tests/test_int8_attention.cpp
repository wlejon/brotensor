...
//   linear_forward_batched_int8w_fp16  vs  linear_forward_batched_fp16(W_deq, X)
//   flash_attention_project_kv_int8w_fp16  vs  flash_attention_project_kv
//   flash_attention_q_with_kv_cached_int8w_fp16  vs  FP16 cached path
//   flash_attention_qkvo_int8w_fp16  vs  flash_attention_qkvo_forward
...
static void test_linear_batched_int8w() {
    std::printf("  linear_forward_batched_int8w_fp16 B=12 in=128 out=192 (+bias)\n");
    ...
    Tensor W_int8, S, W_deq, X_g, B_g, Y_ref, Y_int8;
    prepare_w8a16(OUT, IN, Wh, W_int8, S, W_deq);
    X_g = Tensor::from_host_fp16_on(Device::CUDA, Xh.data(), B,   IN);
    B_g = Tensor::from_host_fp16_on(Device::CUDA, bh.data(), OUT, 1);

    brotensor::linear_forward_batched_fp16(W_deq, &B_g, X_g, Y_ref);
    brotensor::linear_forward_batched_int8w_fp16(W_int8, S, &B_g, X_g, Y_int8);
    CHECK(Y_int8.dtype == Dtype::FP16 && Y_int8.rows == B && Y_int8.cols == OUT);
    std::vector<uint16_t> ref(B * OUT), got(B * OUT);
    Y_ref.copy_to_host_fp16(ref.data());
    Y_int8.copy_to_host_fp16(got.data());
    brotensor::sync_all();
...
static void test_flash_qkvo_int8w() {
    std::printf("  flash_attention_qkvo_int8w_fp16 Lq=8 Lk=12 D=64 H=2 (cross)\n");
    ...
    Tensor X_g, C_g, Bq, Bk, Bv, Bo;
    X_g = Tensor::from_host_fp16_on(Device::CUDA, Xh.data(),  Lq, D);
    C_g = Tensor::from_host_fp16_on(Device::CUDA, Ch.data(),  Lk, D_ctx);
    Bq  = Tensor::from_host_fp16_on(Device::CUDA, bqh.data(), D, 1);
    Bk  = Tensor::from_host_fp16_on(Device::CUDA, bkh.data(), D, 1);
    Bv  = Tensor::from_host_fp16_on(Device::CUDA, bvh.data(), D, 1);
    Bo  = Tensor::from_host_fp16_on(Device::CUDA, boh.data(), D, 1);

    Tensor Wq_i8, Wk_i8, Wv_i8, Wo_i8;
    Tensor sq, sk, sv, so;
    Tensor Wq_deq, Wk_deq, Wv_deq, Wo_deq;
    ...
    brotensor::flash_attention_qkvo_int8w_fp16(
        X_g, &C_g,
    ...
    O_ref.copy_to_host_fp16(ref.data());
    O_int8.copy_to_host_fp16(got.data());
    brotensor::sync_all();
    const float me = max_abs_err(got, ref);
    std::printf("    max_err=%g\n", me);
...
static void test_flash_split_int8w() {
    ...
    Tensor X_g = Tensor::from_host_fp16_on(Device::CUDA, Xh.data(), L, D);

    Tensor Wq_i8, Wk_i8, Wv_i8, Wo_i8, sq, sk, sv, so;
    Tensor Wq_deq, Wk_deq, Wv_deq, Wo_deq;
    prepare_w8a16(D, D, Wqh, Wq_i8, sq, Wq_deq);
    ...
    Tensor K_ref, V_ref, O_ref;
    brotensor::flash_attention_project_kv(X_g, Wk_deq, nullptr, Wv_deq, nullptr,
                                          K_ref, V_ref);
    ...
    // INT8 path.
    Tensor K_i8, V_i8, O_i8;
    brotensor::flash_attention_project_kv_int8w_fp16(
        X_g, Wk_i8, sk, nullptr, Wv_i8, sv, nullptr, K_i8, V_i8);
    brotensor::flash_attention_q_with_kv_cached_int8w_fp16(
    ...
    O_ref.copy_to_host_fp16(ref.data());
    O_i8.copy_to_host_fp16(got.data());
    brotensor::sync_all();
    const float me = max_abs_err(got, ref);
    std::printf("    max_err=%g\n", me);
    CHECK(me < 5e-2f);
