...
void launch_resample_bwd_simple(id<MTLComputePipelineState> pso_fp16,
    ...
    const uint32_t Ho = static_cast<uint32_t>(H_out);
    const uint32_t Wo = static_cast<uint32_t>(W_out);
    id<MTLComputePipelineState> pso = (dY.dtype == Dtype::FP16) ? pso_fp16 : pso_fp32;
...
void upsample_bilinear_2x_backward(const Tensor& dY,
    ...
    @autoreleasepool {
        ...
        id<MTLCommandBuffer> cmd = new_command_buffer();
        id<MTLComputeCommandEncoder> enc = [cmd computeCommandEncoder];
        id<MTLComputePipelineState> pso_scatter = (dY.dtype == Dtype::FP16)
            ? pso_up_bilinear_bwd_fp16() : pso_up_bilinear_bwd_fp32();
        ...
        if (dY.dtype == Dtype::FP16) {
            // Fold FP32 scratch back into FP16 dX (overwrite).
            id<MTLComputePipelineState> pso_copy = pso_copy_fp32_to_fp16();
