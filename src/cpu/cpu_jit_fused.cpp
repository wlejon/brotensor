...
struct JitFusedKernels {
    ...
    using FusedResidualLayerNormRowFn = void (*)(float* x, const float* res, const float* gamma, const float* beta, float* y, uint64_t d, float eps, float inv_d);
    FusedResidualLayerNormRowFn fused_residual_layernorm_row_fn = nullptr;
...
// ── 2. Fused Residual LayerNorm Row (X += res in-place + LayerNorm) ─────────
//
// Computes in-place residual addition X += res, writes X, and computes mean and
// variance across vector registers, writing Y = gamma * (X - mean) * rstd + beta
// directly in registers without intermediate buffers.
static KernelFunction build_fused_residual_layernorm_row(KernelJit& jit) {
    Module mod("mod_fused_residual_layernorm_row");
    mod.add_external_symbol("rsqrtf");
    Function* fn = mod.create_function("fused_residual_layernorm_row", Type::void_type(), {
        Type::ptr(), Type::ptr(), Type::ptr(), Type::ptr(), Type::ptr(), Type::i64(), Type::f32(), Type::f32()
    });
    KernelBuilder kb(mod, fn);
    ...
    // Pass 1 blocks (residual add + mean)
    BasicBlock* l1_vhead = kb.builder().create_block("res_ln_l1_vhead");
    BasicBlock* l1_vbody = kb.builder().create_block("res_ln_l1_vbody");
    BasicBlock* l1_vexit = kb.builder().create_block("res_ln_l1_vexit");
    BasicBlock* l1_shead = kb.builder().create_block("res_ln_l1_shead");
    BasicBlock* l1_sbody = kb.builder().create_block("res_ln_l1_sbody");
    BasicBlock* l1_exit = kb.builder().create_block("res_ln_l1_exit");

    // Pass 2 blocks (variance)
    BasicBlock* l2_vhead = kb.builder().create_block("res_ln_l2_vhead");
    BasicBlock* l2_vbody = kb.builder().create_block("res_ln_l2_vbody");
    BasicBlock* l2_vexit = kb.builder().create_block("res_ln_l2_vexit");
    BasicBlock* l2_shead = kb.builder().create_block("res_ln_l2_shead");
    BasicBlock* l2_sbody = kb.builder().create_block("res_ln_l2_sbody");
    BasicBlock* l2_exit = kb.builder().create_block("res_ln_l2_exit");

    // Pass 3 blocks (gamma * xhat + beta)
    BasicBlock* l3_vhead = kb.builder().create_block("res_ln_l3_vhead");
    BasicBlock* l3_vbody = kb.builder().create_block("res_ln_l3_vbody");
    BasicBlock* l3_shead = kb.builder().create_block("res_ln_l3_shead");
    BasicBlock* l3_sbody = kb.builder().create_block("res_ln_l3_sbody");
    BasicBlock* l3_exit = kb.builder().create_block("res_ln_l3_exit");
    ...
    // Pass 1: Vector sum + in-place residual addition X += res
    fn->append_block(l1_vhead);
    Value* iv1_v = kb.builder().add_block_param(l1_vhead, Type::i64());
    ...
    Value* vx_new = kb.vadd(vx1, vr1);
    kb.vstore_f32x8(px1, vx_new); // X += res in-place!
    Value* next_vsum1 = kb.vadd(vsum1, vx_new);
    Value* next_iv1_v = kb.builder().build_add(iv1_v, eight_i64);
    ...
    Value* ri1 = kb.load_f32_indexed(res, iv1_s, 4, 0);
    Value* xi_new = kb.add(xi1, ri1);
    kb.store_f32_indexed(x, iv1_s, xi_new, 4, 0); // X += res in-place!
    Value* next_sum1 = kb.add(sum1, xi_new);
    Value* next_iv1_s = kb.builder().build_add(iv1_s, one_i64);
    ...
    // Pass 3: Vector LayerNorm Y = gamma * (X - mean) * rstd + beta
    ...
    Value* vy3 = kb.vfma(vg3, vxhat3, vb3); // y = gamma * xhat + beta
    ...
    return jit.compile(mod, "fused_residual_layernorm_row");
}

// ── 3. Fused LayerNorm + Modulate Row (Option B: Zero Intermediate Memory) ──
//
...
static const JitFusedKernels& get_fused_kernels() {
    ...
    std::call_once(init_flag, [] {
        try {
            ...
            // 2. Fused Residual LayerNorm row
            KernelFunction k_fused_res_ln = build_fused_residual_layernorm_row(jit);
            kernels.fused_residual_layernorm_row_fn = k_fused_res_ln.as<JitFusedKernels::FusedResidualLayerNormRowFn>();
            kernels.engines.push_back(k_fused_res_ln.engine());

            // 3. Fused LayerNorm Modulate row (Option B)
            ...
            kernels.available = (kernels.fused_residual_rmsnorm_row_fn != nullptr &&
                                 kernels.fused_residual_layernorm_row_fn != nullptr &&
...
void fused_residual_layernorm(float* X, const float* res, const float* gamma, const float* beta,
    ...
    if (B > 1 && static_cast<int64_t>(B) * D >= 262144) {
        detail::cpu::parallel_for(static_cast<std::size_t>(B), [&](std::size_t bi) {
            ...
            const float* rr = res + static_cast<std::size_t>(b) * D;
            float* yr = Y + static_cast<std::size_t>(b) * D;
            k.fused_residual_layernorm_row_fn(xr, rr, gamma, beta, yr, static_cast<uint64_t>(D), eps, inv_D);
        });
    } else {
        for (int b = 0; b < B; ++b) {
            float* xr = X + static_cast<std::size_t>(b) * D;
