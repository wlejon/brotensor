...
#define BROTENSOR_FOR_EACH_OP(X)                                                                                                                                       \
    ...
    X(copy_d2d_strided,                        void,  (const ::brotensor::Tensor& src, int src_off, int src_pitch,                                                      \
    ...
    /* align_corners=True variant (forward only — inference resample for DPT     \
       depth / segmentation heads). Implemented on all three backends. */         \
    ...
    X(self_attention_decomposed_rel_pos_windowed_forward, void, (const ::brotensor::Tensor& X, \
        const ::brotensor::Tensor& Wq, const ::brotensor::Tensor* bq, \
        const ::brotensor::Tensor& Wk, const ::brotensor::Tensor* bk, \
        const ::brotensor::Tensor& Wv, const ::brotensor::Tensor* bv, \
        const ::brotensor::Tensor& Wo, const ::brotensor::Tensor* bo, \
        const ::brotensor::Tensor& rel_pos_h, const ::brotensor::Tensor& rel_pos_w, \
        int num_heads, int grid_h, int grid_w, int window, float scale, ::brotensor::Tensor& O)) \
