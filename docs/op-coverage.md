...
| `spatial.h` | pad2d, slice2d, unfold2d (neighborhood im2col), window partition/reverse (SAM), spatial 2×2 patch merge (Qwen-VL block-major + `pixel_unshuffle` channel-major), patch_unpack (DiT unpatchify), pixel_shuffle_upsample_2x (DC-AE up-shortcut), NCHW↔sequence transpose |
| `resize.h` | 2× nearest/bilinear up (+ bwd) — 2× avg down lives in `pooling.h` — arbitrary-scale interp2d (nearest/bilinear/bicubic, half-pixel + align-corners), convex (RAFT) upsample, 1D resample |
...
| `loss.h` | softmax (+ bwd), row-batched softmax_rows, softmax-xent (+ segment / fused / fused-batched), MSE (vec / scalar / per-sample), BCE-with-logits fused-batched |
| `optim.h` | sgd_step, adam_step, xavier_init |
| `elementwise.h` | add / scale / clamp / mul-inplace, axpby / add_scalar / add_channel_bias in-place, dtype `cast` (FP32↔FP16/BF16), log / exp / round (+ bwd), sin / cos / rsqrt (+ bwd), threshold_u8 (binary threshold → INT8 byte mask, SAM AMG) |
| `sampling.h` | `sample_logits` (temperature / top-k / top-p / greedy) + `sample_logits_into` (caller-owned buffers, CUDA-graph-safe), Philox RNG (`randn` / `rand_uniform` / `rand_bernoulli` / `randn_truncated`) |
...
|---|---|---|---|---|
| matmul | ✓ | ✓ | ✓ | plain row-major `A @ B` (no bias); dtype-dispatched FP32 + FP16 (FP32 accumulation); backward returns dA/dB (caller zeros, op accumulates; FP16 uses FP32 scratch + fold). Metal FP32/FP16/BF16 (matmul, matmul bwd, batched linear fwd/bwd incl. FP16-weight/FP32-activation, matmul_abt, 1x1 conv): one FP32-accumulate simdgroup GEMM, 64×64 tiles (half tiles BK 32 for FP16×FP16, float tiles BK 16 otherwise), any layout/shape, M≤8 skinny GEMV-style kernel (`src/metal/gemm_fp32.mm`); Metal matmul bwd accumulates in the GEMM epilogue (one rounding, no scratch) |
| matmul_abt | — | — | ✓ | batched strided `A @ Bᵀ`: `C[b](M,N) = A[b](M,K) @ B[b](N,K)ᵀ` with caller-supplied per-slice element strides; 16-bit only (FP16 or BF16), FP32 accumulation, optional fused bias + activation epilogue; backs attention QK^T/PV and Linear. `C` is **not** auto-resized — the caller pre-sizes and dtypes it. Metal FP16 / BF16: the simdgroup GEMM in `src/metal/gemm_fp32.mm` (M≤8 or M·N≥1024, masked so any shape is valid), naive fallback below that. CUDA WMMA equivalent |
...
| relu / tanh / sigmoid | ✓ | ✓ | ✓ | elementwise; GPU dtype-dispatched FP32/FP16/BF16 on both the unary and batched fwd+bwd paths (relu/tanh have batched variants) |
...
| axpby / add_scalar / add_channel_bias (in-place) | ✓ | n/a | ✓ | in-place `y = a*x + b*y`, scalar broadcast add, and per-channel NCHW bias add; dtype-dispatched |
...
| threshold_u8 | ✓ | n/a | ✓ | binary threshold → INT8 byte mask (SAM AMG mask binarization); FP32/FP16 input |
| build_slot_mask | ✓ | n/a | — | device-side validity mask construction |
| softmax | ✓ | ✓ | — | masked, numerically stable |
| softmax_rows | ✓ | n/a | ✓ | row-batched stable softmax — one launch covers all `rows`, replacing a per-row `softmax` loop (attention score matrices). May run in place (`Y == X`) |
...
| flash_attention_gqa | ✓ | — | ✓ | grouped-query self-attention over pre-projected Q/K/V, **causal or bidirectional**. Q carries `num_q_heads`, K/V carry `num_kv_heads` (must divide; equal == plain MHA). `causal=false` is full bidirectional attention over all Lk keys — the encoder prefill a bidirectional-ified decoder needs (LLM2Vec); `causal=true` reproduces `flash_attention_forward`'s mask with GQA. Tiled online softmax, FP32/FP16/BF16, FP32 accumulation; optional length-Lk key mask. Forward-only, all three backends |
...
| flash_attention_decode | — | — | ✓ | causal-aware decode against a partially-filled K/V cache; supports `L_q ≥ 1` (token-by-token or chunked). Masked variant `flash_attention_decode_masked` (per-key FP32 validity mask, `L_q == 1`) on all three backends |
...
| masked_mean_pool | ✓ | ✓ | ✓ | row-wise mean over valid mask; GPU dtype-dispatched FP32/FP16/BF16 (fwd + bwd) |
...
| rows_count_above | ✓ | n/a | ✓ | per-row above-threshold counts at two thresholds in one pass → (R,2) INT32 (SAM AMG stability score); FP32/FP16 input |
...
| copy_d2d | ✓ | n/a | ✓ | flat-buffer device-to-device chunk copy. Strided variant `copy_d2d_strided` (pitched 2D copy, e.g. NCHW W-axis pad/unpad) on all three backends |
...
| conv_transpose1d / 2d | ✓ | ✓ | — | learned upsample (vocoders, SAM mask decoder, DPT heads); FP32, all three backends |
...
| patch_unpack | ✓ | n/a | ✓ | DiT unpatchify — (hp*wp, P*P*C_total) tokens → NCHW, block-major or channel-major column composition; channels past `C_keep` dropped. Inference-only; FP32/FP16/BF16 |
| pixel_shuffle_upsample_2x | ✓ | n/a | ✓ | fused `repeat_interleave` + `pixel_shuffle(2)` (DC-AE up-shortcut) with no intermediate 4·C_out tensor. `C_in == 4·C_out` is a plain pixel-shuffle; `C_in == C_out` reduces to 2× nearest. Pure gather, inference-only; FP32/FP16/BF16 |
...
| max_pool2d / adaptive_avg_pool2d | ✓ | ✓ | — | indexed max-pool bwd + PyTorch adaptive avg pool; FP32, all three backends |
...
| Sampling | `sample_logits` — temperature / top-k / top-p autoregressive sampler, counter-based Philox RNG. `sample_logits_into` is the same sampler over caller-owned scratch/counter/index buffers, so a decode step allocates nothing and stays CUDA-graph-capturable |
